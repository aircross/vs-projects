...
namespace POSDLL_V1
{
    ...
    public class C_POSDLL
    {
        ...
        public static int POS_SetMode(int nPrintMode)
        {
            ...
            switch (nPrintMode)
            {
                case POS_PRINT_MODE_STANDARD:
                    data = POSCOMMAND.ESC_S;
                    break;
                case POS_PRINT_MODE_PAGE:
                    data = POSCOMMAND.ESC_L;
                    break;
                default:
                    data = POSCOMMAND.ERROR;
                    break;
            }
            if (POS_WriteFile(hPort, data, data.Length))
                return POS_SUCCESS;
        ...
        public static int POS_PL_SetArea(int nOrgx, int nOrgy, int nWidth, int nHeight, int nDirection)
        {
            ...
            int dataLength = POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH.Length + POSCOMMAND.ESC_T_n.Length;

            byte[] data = new byte[dataLength];
            int offset = 0;
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[2] = (byte)(nOrgx % 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[3] = (byte)(nOrgx / 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[4] = (byte)(nOrgy % 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[5] = (byte)(nOrgy / 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[6] = (byte)(nWidth % 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[7] = (byte)(nWidth / 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[8] = (byte)(nHeight % 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH[9] = (byte)(nHeight / 0x100);
            POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH.CopyTo(data, offset);
            offset += POSCOMMAND.ESC_W_xL_xH_yL_yH_dxL_dxH_dyL_dyH.Length;
            POSCOMMAND.ESC_T_n[2] = (byte)(nDirection & 0x03);
            POSCOMMAND.ESC_T_n.CopyTo(data, offset);
        ...
        public static int POS_PL_TextOut(string pszString, int nOrgx, int nOrgy, int nWidthTimes, int nHeightTimes, int nFontType, int nFontStyle)
        {
            ...
            byte[] pbString = Encoding.Default.GetBytes(pszString);
            int dataLength = pbString.Length + POSCOMMAND.GS_backslash_nL_nH.Length + POSCOMMAND.GS_dollors_nL_nH.Length +
                POSCOMMAND.GS_exclamationmark_n.Length + POSCOMMAND.ESC_M_n.Length + POSCOMMAND.GS_E_n.Length +
                POSCOMMAND.ESC_line_n.Length + POSCOMMAND.GS_B_n.Length;
            ...
            POSCOMMAND.ESC_dollors_nL_nH.CopyTo(data, offset);
            offset += POSCOMMAND.ESC_dollors_nL_nH.Length;
            POSCOMMAND.GS_dollors_nL_nH[2] = (byte)(nOrgy % 0x100);
            POSCOMMAND.GS_dollors_nL_nH[3] = (byte)(nOrgy / 0x100);
            POSCOMMAND.GS_dollors_nL_nH.CopyTo(data, offset);
            offset += POSCOMMAND.GS_dollors_nL_nH.Length;
            byte[] intToWidth = { 0x00, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70 };
            byte[] intToHeight = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 };
            POSCOMMAND.GS_exclamationmark_n[2] = (byte)(intToWidth[nWidthTimes] + intToHeight[nHeightTimes]);
            POSCOMMAND.GS_exclamationmark_n.CopyTo(data, offset);
        ...
        {
            if (nOrgx < 0 | nOrgx > 65535 | nOrgy < 0 | nOrgy > 65535)
                return POS_ERROR_INVALID_PARAMETER;
            ...
            int dataLength = POSCOMMAND.ESC_dollors_nL_nH.Length + POSCOMMAND.GS_dollors_nL_nH.Length + POSCOMMAND.GS_backslash_m.Length;
            byte[] data = new byte[dataLength];
            int offset = 0;
            POSCOMMAND.ESC_dollors_nL_nH[2] = (byte)(nOrgx % 0x100);
            POSCOMMAND.ESC_dollors_nL_nH[3] = (byte)(nOrgx / 0x100);
        ...
        /// <param name="nMode"></param>
        /// <returns></returns>
        private static int POS_PL_PrintBmpInFLASH(int nID, int nOrgx, int nOrgy, int nMode)
        {
            if (nID < 1 | nID > 255 | nOrgx < 0 | nOrgx > 65535 | nOrgy < 0 | nOrgy > 65535)
                return POS_ERROR_INVALID_PARAMETER;
            ...
            int dataLength = POSCOMMAND.ESC_dollors_nL_nH.Length + POSCOMMAND.GS_dollors_nL_nH.Length + POSCOMMAND.FS_p_n_m.Length;
            byte[] data = new byte[dataLength];
            int offset = 0;
            POSCOMMAND.ESC_dollors_nL_nH[2] = (byte)(nOrgx % 0x100);
            POSCOMMAND.ESC_dollors_nL_nH[3] = (byte)(nOrgx / 0x100);
        ...
        {
            if (nOrgx < 0 | nOrgx > 65535 |nOrgy<0|nOrgy>65535| nType < 0x41 | nType > 0x49 | nWidthX < 2 | nWidthX > 6 | nHeight < 1 | nHeight > 255 | (pszInfoBuffer.Length != nBytesToPrint))
                return POS_ERROR_INVALID_PARAMETER;
            ...
            byte[] pbString = Encoding.Default.GetBytes(pszInfoBuffer);
            int dataLength = POSCOMMAND.ESC_dollors_nL_nH.Length + POSCOMMAND.GS_dollors_nL_nH.Length + POSCOMMAND.GS_w_n.Length +
                POSCOMMAND.GS_h_n.Length + POSCOMMAND.GS_f_n.Length + POSCOMMAND.GS_H_n.Length + POSCOMMAND.GS_k_m_n_.Length + pbString.Length;
