...
namespace POSDLL_V2
{
    ...
    public class C_POSDLL
    {
        ...
        public const int POS_FONT_STYLE_ALIGN_CENTER = 0x4000;
        public const int POS_FONT_STYLE_ALIGN_RIGHT = 0x8000;
        ...
        public const int POS_BARCODE_TYPE_QRCODE = 0x61;
        ...
        public static int POS_NETPORT_HANDLE = -1;
        ...
        private class IOCONTROL
        {
            ...
            [DllImport("kernel32.dll", SetLastError = true)]
            public static extern bool ReadFile(int hFile, byte[] lpBuffer, uint nNumberOfBytesToRead, ref uint lpNumberofBytesRead, int lpOverlapped);
            [DllImport("kernel32.dll", SetLastError = true)]
        ...
        private class POSCOMMAND
        {
            ...
            /// n=4：传送纸传感器状态
            /// </summary>
            public static byte[] DLE_EOT_n = { 0x10, 0x04, 0x01 };

            /// <summary>
            /// 参照三星打印机
            /// </summary>
            public static byte[] GS_leftbracket_A_pL_pH_n_m = { 0x1d, 0x28, 0x41, 0x02, 0x00, 0x00, 0x02 };
        }
        ...
        public static bool POS_WriteFile(int hPort, byte[] bDataSendBuffer, int nBytesToWrite)
        {
            ...
            else if (hPort == POS_NETPORT_HANDLE)
            {
                sc.Send(bDataSendBuffer);
                return true;
            }
            else
                return false;
        ...
        public static bool POS_WriteFile(int hPort, byte[] bDataSendBuffer)
        {
            int nBytesToWrite = bDataSendBuffer.Length;
            if (hPort == POS_COM_HANDLE)
            {
                IOCONTROL.OVERLAPPED tempOverLapped = new IOCONTROL.OVERLAPPED();
                uint BytesOfWritten = 0;
        ...
        public static int POS_RTQueryStatus(byte[] pszStatus)
        {
            int hPort;
            if (POS_COM_SELECTED & (POS_COM_HANDLE != -1))
                hPort = POS_COM_HANDLE;
            ...
            POSCOMMAND.DLE_EOT_n.CopyTo(data, 0);
            POSCOMMAND.DLE_EOT_n.CopyTo(data, 3);
            POSCOMMAND.DLE_EOT_n.CopyTo(data, 6);
            POSCOMMAND.DLE_EOT_n.CopyTo(data, 9);
            data[5] = 0x02;
            data[8] = 0x03;
            data[11] = 0x04;
            ...
            if (POS_WriteFile(hPort, data, data.Length))
            {
                ...
                if (IOCONTROL.ReadFile(POS_COM_HANDLE, buffer, (uint)buffer.Length, ref p, 0))
                {
                    status = (byte)(((buffer[0] & 0x0c) >> 2) +
                        (buffer[1] & 0x0c) + ((buffer[1] & 0x40) >> 2) +
                        ((buffer[2] & 0x08) << 2) +
                        ((buffer[3] & 0x08) << 3) + ((buffer[3] & 0x80) << 1));
        ...
        public static string POS_RTQueryStatus()
        {
            int hPort;
            if (POS_COM_SELECTED & (POS_COM_HANDLE != -1))
                hPort = POS_COM_HANDLE;
            ...
            if (POS_WriteFile(hPort, data, data.Length))
            {
                ...
                {
                    for (int i = 0; i < 4; i++)
                        status += COMMUNICATION.Status.Sta_ReturnStatus(buffer[i], (i + 1));
                    return status;
                }
            }
            return string.Empty;
        ...
        internal static int POS_PL_Test()
        {
            int hPort;
            if (POS_COM_SELECTED & (POS_COM_HANDLE != -1))
                hPort = POS_COM_HANDLE;
